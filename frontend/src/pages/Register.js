    ...
    <div className='w-full h-[100vh] bg-slate-200 flex items-center justify-center'>
      <div className='w-full max-w-[500px] h-[600px] bg-white rounded-lg shadow-gray-400 shadow-lg flex flex-col gap-[40px]'>
        <div className='w-full h-[200px] bg-[#53abe1] rounded-b-[20%] shadow-gray-400 shadow-lg flex items-center justify-center'>
          <h1 className='text-gray-300 text-2xl font-bold '>Welcome To <span className='text-[#142c36bb]'>HeyBuddy</span></h1>
        </div>
        <form className='flex flex-col gap-[20px] items-center' onSubmit={submithandler}>
          <input type='text' placeholder='username' value={username} onChange={(e)=>{setUsername(e.target.value)}}  className='w-[90%] h-[50px] py-2 px-4 border-2 border-[#53abe1] rounded-xl outline-none shadow-gray-400 shadow-lg text-gray-700 text-[19px]'/>
          ...
          <input type={`${show ? 'text':'password'}`}  value={password} onChange={(e)=>{setPassword(e.target.value)}}  placeholder='password' className='outline-none'/>
          ...
          <button className='w-[90%] h-[50px] rounded-xl bg-[#53abe1] text-[#142c36bb] font-bold text-xl mt-4 shadow-gray-400 shadow-lg hover:shadow-inner' >{`${loading?"Loading...":"Register"}`}</button>
          <p className='cursor-pointer'>Already have an account? <span className='text-blue-700 font-bold' onClick={()=>{navigate('/login')}}>Login</span></p>
        </form>
